...
\emph{Free/Open-Source Software}, que diz respeito aos programas que são
licenciados livremente para garantir aos usuários o direito de uso, estudo e
modificação, por meio da disponibilidade do seu código-fonte.}), que propiciam
a construção coletiva de uma ampla gama de softwares de qualidade em constante
atualização e evolução, organizados na forma de um rossio \cite{Simon:08}.
...
indicação que é bastante popular nas relações sociais. Tradicionalmente as
recomendações eram produzidas exclusivamente por especialistas na área em que
se pretendia recomendar \cite{Resnick:97}.
%FIXME [Confirmar citacao]
A recomendação de filmes em cartaz é um exemplo deste tipo de indicação, publicada por críticos de arte
...
\item[Navegação.] Muitos usuários utilizam estes sistemas mesmo quando não têm
...
%FIXME [Revisar paragrafo]
Por outro lado, alguns termos tem pouco poder de discriminação na determinação
...
%Por exemplo, numa coleção de documentos sobre
%filmes, o termo \emph{diretor} aparece em quase todos os documentos, ao passo
%que outros são raríssimos, por exemplo, \emph{} outros são raros. Considerando
%apenas a medida $tf$, 
%
% e a medida \emph{tf} não considera isto.
...
propostas ao longo do tempo por pesquisadores na temática de recuperação de
informação com o intuito de resolver anomalias e aperfeiçoar a busca.
Por exemplo, é pouco provável que a persença de uma palavra 20 vezes num
...
\begin{table}[h]
  ...
  \begin{tabular}{| l | l |}
    \hline
    Distância euclidiana & $d(X,Y) = \sqrt{(x_1-y_1)^2+(x_2-y_2)^2+...+(x_n-y_n)^2}$\\
    \hline
    ...
    \hline
    Coeficiente de Pearson & $p(X,Y) = \frac{\sum_{1 \leq i \leq n} (x_i-\bar{x})(y_i-\bar{y})}{\sqrt{\sum_{1 \leq i \leq n} (x_i-\bar{x})^2}\times\sqrt{\sum_{1 \leq i \leq n} (y_i-\bar{y})^2}}$\\
    \hline
  \end{tabular}
\end{table}
...
dos usuários em suas opiniões, que de fato não pode ser verificada devido a seu
caráter subjetivo e estritamente pessoal. Atualmente existem serviços
especializados em reputação de produtos, que não realizam venda associada,
apenas disponibilizam as avaliações. É o caso do \emph{Internet Movie Database}
\footnote{\url{http://www.imdb.com/}}, apresentado na figura \ref{fig:imdb}.
...
recuperação da informação para encontrar itens semelhantes (por exemplo,
\emph{tf-idf} e \emph{BM25}) ou de classificação para encontrar itens
relevantes (por exemplo, \emph{naïve Bayes}). Em uma livraria, sugerir
ao cliente outros livros do mesmo autor ou tema de livros previamente
...
recomendações se limitar a itens similares aos já escolhidos pelos usuários.

\subsubsection{Colaborativa}

Esta estratégia não exige a compreensão ou reconhecimento do conteúdo dos itens,
pois se baseia na troca de experiências entre pessoas que possuem interesses
em comum. A figura \ref{fig:rec_colaborativa} ilustra o cenário da recomendação
colaborativa.
...
A estratégia do K-NN é comummente utilizada neste tipo de solução. Define-se
uma função que representa a distância entre os usuários, e a vizinhança de um
usuário $u$ é composta pelos que estiverem mais próximos dele (com menor
distância). Porém, ao invés de uma classe ser extraída da vizinhança, como
descrito na seção \ref{sec:knn}, uma recomendação para o usuário $u$ é
produzida a partir da análise dos itens que a vizinhança de $u$ considera
relevantes. Geralmente os itens que aparecerem com maior frequência na
vizinhança compõem a recomendação.
...
todos os casos, e sim numa porcentagem mínima, definida pelo suporte.

A implementação desta estratégia é mais complexa, pois exige técnicas mais
...
Um exemplo de recomendação por associação é encontrado no site da Amazon
...
cada atributo. Ademais, diferentes estratégias podem funcionar melhor ou pior
de acordo com o domínio da aplicação e as propriedades dos dados. Por exemplo,
...
domínios onde há muito mais itens do que usuários. \cite{Herlocker:2004}
sugere que o processo de avaliação tenha início com a compreensão das ações
para as quais o sistema foi projetado (seção \ref{sec:acoes}), como guia para
as decisões metológicas ao longo dos testes.
...
conclusões comparativas é arriscado, visto que os dados produzidos podem se
ajustar melhor para uma estratégia do que para outras \cite{Herlocker:2004}.
...
operacional. Porém, o núcleo do sistema (\emph{GNU Hurd}) teve desenvolvimento
mais lento.
...
softwares selecionados por seus desenvolvedores. As distribuições reduzem a
...
comunidade, como descrito acima. Este é um ponto chave no desenvolvimento,
pois é um dos fatores que mais influenciam a escolha dos usuários pela
distribuição. O impacto que tal seleção tem para a comunidade de usuários
resulta em frequentes polêmicas em torno do tema, como a gerada pelo
anúncio de que o projeto \emph{Ubuntu} abandonaria o \emph{Gnome} como
interface padrão de usuário \cite{Paul:10}.

Softwares adicionais para atender a necessidades específicas dos usuários
...
As seguir estão descritos os fatores que contribuiram para a seleção do
...
instalação, atualização e remoção de pacotes são disparadas pelo \emph{APT}, que
num nível mais baixo faz uso do \emph{dpkg}, ferramenta que de fato realiza
instalações e remoções de softwares. O \emph{APT} também gerencia conflitos
entre pacotes, checando se novos pacotes a serem instalados conflitam com algum
...
\item Segundo o \emph{contrato
social}\footnote{\url{http://www.debian.org/social_contract.pt.html}} do Debian
perante a comunidade de software livre, o desenvolvimento do projeto é guiado
pelas necessidades dos usuários e da comunidade. Portanto, as iniciativas de
colaboradores individuais, seja ele desenvolvedor oficial ou não, serão
igualmente consideradas e passarão a fazer parte da distribuição se seguirem
aqueles princípios e forem considerados de utilidade para a comunidade.
...
A maioria das as distribuições GNU/Linux têm investido no desenvolvimento de
...
recentes iniciativas pioneiras no campo de gerenciamento de aplicações
\cite{Zini:11}. Diante da complexa e crescente estrutura do projeto,
observa-se um esforço por parte dos desenvolvedores, principalmente da equipe
responsável pelo controle de qualidade\footnote{\url{http://qa.debian.org}}, de
reunir, organizar e diponibilizar as informações ou meta-dados concernentes a
esta estrutura \cite{Nussbaum:10}.
...
ao passo se mostraram úteis e eficientes foram absorvidas pela comunidade de
...
\begin{table}[h]
  ...
  \begin{tabularx}{25cm}{| c | X | c |}
    \hline
    \normalsize \textbf{\emph{Solução}} & \normalsize \textbf{\emph{Descrição}} & \normalsize \textbf{\emph{Estratégia de recomendação}}\\
    \hline BTS  & \multirow{3}{13cm}{Sistema de acompanhamento de bugs, alimentado pelos usuários e desenvolvedores da distribuição, reune todo o histórico referente ao relatório e correção de erros em pacotes.} & \multirow{10}{*}{Reputação}\\
    \emph{Bug Tracking System}& & \\
    \scriptsize{\url{http://bugs.debian.org}}& & \\
    \cline{1-2}
    Popcon & \multirow{3}{13cm}{Concurso de popularidade entre pacotes realizado diariamente, disponibiliza estatísticas de uso dos pacotes do repositório. Provê gráficos específicos por pacote, por arquitetura, por desenvolvedor etc.} & \\
    \emph{Popularity Contest}& & \\
    \scriptsize{\url{http://popcon.debian.org}} & & \\
    \cline{1-2}
    PTS & \multirow{3}{13cm}{Sistema de acompanhamento de pacotes, reune informações relativas à manutenção dos pacotes: versão, mantenedor, \emph{upstream}, bugs abertos por tipo, últimas atualizações no repositório etc.} & \\
    \emph{Package Tracking System}& & \\
    \scriptsize{\url{http://packages.qa.debian.org}} & & \\
    & & \\
    \cline{1-2}
    UDD & \multirow{3}{13cm}{{Iniciativa recente do time de qualidade com o intuito de reunir informações de diversos aspectos do Debian numa base de dados única. Usuários avançados podem consultar esta base para tomar decisões acerca de que pacotes usar.}} & \\
    \emph{Ultimate Debian Database}& & \\
    \scriptsize{\url{http://udd.debian.org}}& & \\
    & & \\
    \hline
    Debtags & \multirow{2}{13.5cm}{Caracterização dos pacotes por múltiplos atributos, realizada manualmente por usuários e desenvolvedores, que auxilia a navegação e busca no repositório de pacotes.} & \multirow{2}{*}{Baseada em conteúdo}\\
    Classificação de pacotes & & \\
    \cline{2-3}
    \scriptsize{\url{http://debtags.alioth.debian.org}} & Novas \emph{tags} (atributos) são sugeridas a partir das tags já associadas ao pacote. & Associação\\
    & & \\
    \hline
  \end{tabularx}
\end{table}
...
%FIXME [Descrever o software-center como solução estado-da-arte]
...
Este trabalho se propõe a desenvolver soluções para o problema da recomendação
no contexto de componentes de sofware, em especial no âmbito de distribuições
GNU/Linux, onde os pacotes são modelados como itens e os usuários da
distribuição atuam os clientes do recomendador.
...
um item pode ser relevante ou irrelevante. O cenário da recomendação é então
caracterizado da seguinte maneira: dada a lista de pacotes instalados no
sistema de determinado usuário como representação de sua identidade, o
recomendador deve retornar uma lista de sugestão de pacotes, que representam
pacotes de potencial interesse para tal usuário. Desta forma, a ação
principal do sistema é definida como \emph{encontrar itens relevantes} (seção
\ref{sec:acoes}).

Neste trabalho as recomendações devem ser geradas a partir do comportamento do
usuário. Não serão consideradas informações registradas por usuários no BTS ou
em listas de discussão. Pretende-se calcular a recomendação a partir de dados
do \emph{Popcon}, que contém listas de pacotes instalados de milhares de sistemas
...
explicitamente requisitos em seu conteúdo \cite{Cosmo:09}. Requisitos positivos
representam relações de dependência e os negativos de conflito, podendo ser
definidos em diversos graus: dependência, sugestão, recomendação, conflito,
substituição, quebra, etc \cite{Cosmo:09}. Por exemplo, se um componente $a$
depende de $b$, significa dizer que $b$ deve ser instalado no sistema para
que $a$ funcione como previsto. Por outro lado, se $a$ conflita com $c$, a
...
de pacotes. Numa mesma recomendação, não faz sentido recomendar pacotes
dependentes, a menos que suas características intrínsecas sejam a causa da
recomendação. No caso de pacotes com alguma relação de dependência, o fato de
estarem presentes concomitantemente em grande parte dos sistemas não é uma
coincidência, e sim consequência da relação de dependência.

Por outro lado, teriam grande valor as regras de associação mineradas a partir
da análise dos dados estatísticos relacionando componentes que não apresentam
relação alguma de dependência. Neste cenário, novos graus de relacionamento
poderiam ser estabelecidos, baseados não na dependência, mas na colaboração dos
componentes.
...
preservam os relacionamentos usuário-item, já que as listas de pacotes
originais submetidas pelos usuários não são publicadas. Por questões de
privacidade, as estatísticas são geradas e um resumo de todas as submissões
é disponibilizado diariamente, mas os dados de usuários reais ficam armazenados
nos servidores do projeto e apenas desenvolvedores oficiais têm acesso a esses
dados\footnote{\url{http://popcon.debian.org/FAQ}}. Para a realização deste
trabalho, o apoio do desenvolvedor Debian \emph{Tiago
Vaz}\footnote{\url{http://qa.debian.org/developer.php?login=tiago\%40debian.org}},
permitirá o acesso aos dados necessários.

\subsubsection{Seleção de atributos}

Existem peculiaridades do domínio de componentes de software que, se analisadas
com o devido cuidado, podem revelar ganhos potenciais na eficiência e qualidade
das recomendações. A seleção de atributos geralmente proporciona ganhos,
na medida em que diminui o ruído e a ordem de grandeza dos dados.
...
A última etapa de seleção de atributos utilizá os métodos clássicos
...
no que outros usuários parecidos com ele têm instalado em seus sistemas e
ele não tem.
...
colaborativa a partir das \emph{tags} dos pacotes de cada usuário (descrição
de conteúdo). Outras soluções híbridas podem ser projetadas a medida que os
recomendadores das estratégias puras forem sendo implementados e posteriormente
combinados.
...
\begin{description}
  \item[Areas de interesse:] a composição de um perfil de usuário que indique
  ...
  \item[Popularidade:] pacotes que figuram entre os mais populares no
    \emph{Popcon} devem receber maior pontuação nos cálculos, nem que este seja
    como apenas um critério de desempate;
  \item[Bugs:] muitos bugs abertos são um indicativo de problemas,
    principalmente se forem bugs graves. Devido ao número de pacotes reduzido
    em cada recomendação, deve-se dar prioridade à incluir pacotes que estão
    recebendo atenção frequente de seu mantenedor.
\end{description}
...
Diante da não disponibilidade de dados que possibilitassem a análise
\emph{offline} de acurácia das recomendação, optou-se pela realização de
experimentos diretamente com usuários, por meio de um \emph{survey} eletrônico.
Algumas ferramentas estão sendo avaliadas para a construção do \emph{survey},
entre elas, o \emph{LimeSurvey}\footnote{\url{http://www.limesurvey.org/}}.
