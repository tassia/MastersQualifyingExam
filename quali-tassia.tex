...
Um exemplo de recomendação por associação é encontrado na loja virtual da
empresa \emph{Amazon}\footnote{\url{http://www.amazon.com/}} (figura
\ref{fig:amazon}).
...
\begin{table}[h]
  ...
  \begin{tabularx}{25cm}{| c | X | c |}
    ...
    \hline BTS  & \multirow{3}{13cm}{Sistema de acompanhamento de bugs, alimentado pelos usuários e desenvolvedores da distribuição. Reúne todo o histórico referente ao relatório e correção de erros em pacotes.} & \multirow{10}{*}{Reputação}\\
    ...
    \cline{1-2}
    PTS & \multirow{3}{13cm}{Sistema de acompanhamento de pacotes, reúne informações relativas à manutenção dos pacotes: versão, mantenedor, \emph{upstream}, bugs abertos por tipo, últimas atualizações no repositório etc.} & \\
    \emph{Package Tracking System}& & \\
    \scriptsize{\url{http://packages.qa.debian.org}} & & \\
    & & \\
    \cline{1-2}
    UDD & \multirow{3}{13cm}{{Iniciativa recente do time de qualidade criada com o intuito de reunir informações de diversos aspectos do Debian numa base de dados única. Usuários avançados podem consultar esta base para tomar decisões acerca de que pacotes usar.}} & \\
