...
documentos e na tabela \ref{tab:tfidf} estão os valores de \emph{tf$_{t,d}$} de
...
de documentos que contém o termo $t$. Valores de \emph{idf$_t$} para a coleção
da figura \ref{fig:colecao} são apresentados na tabela \ref{tab:tfidf}.
...
\end{enumerate}
\begin{table}[h]
  \caption{Valores de \emph{tf$_{t,d}$} e \emph{idf$_t$} para documentos da coleção}
  \label{tab:tfidf}
  \centering
  \begin{tabular}{| l | c | c | c | c | c | c | c | c | c |}
    \hline
    \multirow{2}{*}{\backslashbox{\textbf{\emph{Termo}}}
                                 {\textbf{\emph{Documento}}}}&
    \textbf{\small{(1)}}&\textbf{\small{(2)}}&\textbf{\small{(3)}}&
    \textbf{\small{(4)}}&\textbf{\small{(5)}}&\textbf{\small{(6)}}&
    \textbf{\small{(7)}}&\textbf{\small{(8)}}&
    \multirow{2}{*}{\textbf{\emph{idf$_t$}}}\\
    \cline{2-9}
    &\multicolumn{8}{c|}{\textbf{\emph{tf$_{t,d}$}}}&\\
    \hline
...
Considerando os pesos \emph{tf-idf} dos termos do dicionário em cada
...
respectivos pesos \emph{tf-idf} no documento. Tem-se como resultado uma visão
...
\begin{table}[h]
  ...
  \begin{tabular}{| l | l |}
    ...
    \hline
    Coeficiente de \emph{Pearson} & $p(X,Y) = \frac{\T\sum_{1 \leq i \leq n} (x_i-\bar{x})(y_i-\bar{y})}{\sqrt{\sum_{1 \leq i \leq n} (x_i-\bar{x})^2}\times\sqrt{\sum_{1 \leq i \leq n} (y_i-\bar{y})^2}\B}$\\
    \hline
  \end{tabular}
\end{table}
...
do seguinte princípio: qualquer subconjunto de um conjunto frequente também é
um conjunto frequente. Portanto, só devem participar da nova combinação os
elementos que apresentarem um suporte superior ao limite, pois um conjunto que
não é frequente não será jamais subconjunto de um conjunto frequente
\cite{Agrawal:94}.
